...
export async function POST(request: Request) {
  try {
    ...
    const mediaTypeId = (formData.get('mediaTypeId') as string) || '013'
    const mediaId = (formData.get('mediaId') as string) || '0000'

    const result = await saveMediaUpload('radio', file, year, month, day, mediaTypeId, mediaId)
    ...
    return NextResponse.json({ error: 'Failed to upload file: ' + String(error) }, { status: 500 })
  }
}
