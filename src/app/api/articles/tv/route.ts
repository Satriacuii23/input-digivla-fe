...
export async function GET(request: Request) {
  try {
    ...
    let url = `${API_URL}/api/articles/tv?page=${page}&limit=${limit}`
    ...
    if (!res.ok) {
      return NextResponse.json({ error: 'Failed to fetch TV articles' }, { status: res.status })
    }
    ...
    console.error('TV articles API error:', error)
...
export async function POST(request: Request) {
  try {
    ...
    console.error('TV articles POST error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    let url = `${API_URL}/api/articles/tv/check-duplicate`
    ...
    console.error('TV duplicate check error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}