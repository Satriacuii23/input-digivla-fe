...
export async function GET(request: Request) {
  try {
    ...
    let url = `${API_URL}/api/articles/online?page=${page}&limit=${limit}`
    ...
    if (!res.ok) {
      return NextResponse.json({ error: 'Failed to fetch Online articles' }, { status: res.status })
    }
    ...
    console.error('Online articles API error:', error)
...
export async function POST(request: Request) {
  try {
    ...
    const res = await fetch(`${API_URL}/api/articles/online`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
    ...
    console.error('Online articles POST error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    let url = `${API_URL}/api/articles/online/check-duplicate`
    ...
    console.error('Online duplicate check error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}