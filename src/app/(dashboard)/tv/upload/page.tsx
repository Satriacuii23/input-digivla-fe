...
type TVFormData = BroadcastMultiFormValues
...
const LAST_VALUES_KEY = 'tv_last_values'
...
const generateFilePath = (uploadDate: Date | null, mediaId: number | null): string => {
  ...
  return `tv_files/${year}/${month}/${day}/${fileName}`
}

const getDefaultFormData = (): TVFormData => ({
  media_id: null,
  title: '',
  content: '',
  date: new Date(),
...
export default function TVUploadPage() {
  ...
  const [multiFormData, setMultiFormData] = useState<TVFormData[]>([getDefaultFormData()])
  ...
  const checkDuplicate = useCallback(async (checkTitle: string, checkContent: string, mediaId: string | null) => {
    ...
    try {
      ...
      const res = await fetch(`/api/articles/tv?${params.toString()}`, { method: 'PUT', credentials: 'include' })
      if (res.ok) {
        const data = await res.json()
        if (data.exists && data.duplicates?.length > 0) {
          setDuplicateWarning({ show: true, duplicates: data.duplicates })
        } else {
          setDuplicateWarning({ show: false, duplicates: [] })
        }
      }
  ...
  useEffect(() => {
    const fetchTVMedia = async () => {
      setMediaLoading(true)
      try {
        const res = await fetch('/api/media/type/by-id/12', { credentials: 'include' })
        if (res.ok) {
          const data = await res.json()
          setMediaOptions(data.map((m: { media_id: number; media_name: string }) => ({
  ...
  const updateMultiFormField = useCallback((index: number, field: keyof TVFormData, value: unknown) => {
    setMultiFormData(prev => {
      const currentForm = prev[index]
      if (!currentForm) return prev
      const updatedForm = { ...currentForm, [field]: value } as TVFormData
      if ((field === 'date' || field === 'media_id') && currentForm.file) {
        updatedForm.filePath = generateFilePath(
          field === 'date' ? (value as Date | null) : currentForm.date,
          field === 'media_id' ? (value ? parseInt(value as string) : null) : (currentForm.media_id ? parseInt(currentForm.media_id) : null)
        )
      }
      const newData = [...prev]
  ...
  const checkMultiFormDuplicatesAndReturn = useCallback(async () => {
    ...
    try {
      const formTitles = multiFormData
        .map((form, idx) => ({ idx, title: form.title.trim(), content: form.content.trim(), media_id: form.media_id }))
        .filter(f => f.title)
      ...
      for (const form of formTitles) {
        ...
        if (form.media_id) params.append('media_id', form.media_id)
        const res = await fetch(`/api/articles/tv?${params.toString()}`, { method: 'PUT', credentials: 'include' })
        if (res.ok) {
          const data = await res.json()
          if (data.exists && data.duplicates?.length > 0) {
            duplicatesByForm.set(form.idx, data.duplicates)
            hasDuplicates = true
          }
        }
      }
  ...
  const submitSelectedArticles = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedArr.length; i++) {
        ...
        if (form.file) {
          const uploadResult = await uploadMediaVideoFile('tv', form.file, form.date, form.media_id!)
        ...
        const payload = {
          ...
          timee: formatWibTimeDisplay(form.time),
          journalist: form.journalist || '',
          duration: form.duration ? String(form.duration) : '',
          filee: finalFilePath,
        }
        const res = await fetch('/api/articles/tv', {
          method: 'POST',
  ...
  const handleSingleSubmit = async () => {
        ...
        modal.confirm({
          ...
          content: (
            <div>
              ...
              <ul>{duplicateWarning.duplicates.slice(0, 3).map((d, i) => <li key={i}>&quot;{d.title}&quot; ({d.media_name}, {d.datee})</li>)}</ul>
              <p>Continue upload?</p>
            </div>
          ),
          okText: 'Continue',
    ...
    try {
      ...
      if (file) {
        const uploadResult = await uploadMediaVideoFile('tv', file, date, selectedMedia!)
      ...
      const payload = {
        media_id: parseInt(selectedMedia!),
        title: title.trim(),
        content: content.trim(),
        datee: (date || new Date()).toISOString().split('T')[0],
        timee: formatWibTimeDisplay(time),
        journalist: anchor || '',
      ...
      if (res.ok) {
        saveLastValues()
        message.success('TV article uploaded successfully')
  ...
  const previewColumns: ColumnsType<{ index: number; form: TVFormData }> = [
    { title: '#', dataIndex: 'index', width: 48, render: (v: number) => v + 1 },
    { title: 'Media', dataIndex: 'form', width: 120, render: (form: TVFormData) => mediaOptions.find(m => m.value === form.media_id)?.label || form.media_id || '-' },
    { title: 'Title', dataIndex: 'form', width: 180, render: (form: TVFormData) => form.title || '-' },
    { title: 'Content', dataIndex: 'form', width: 200, ellipsis: true, render: (form: TVFormData) => form.content || '-' },
    { title: 'Date', dataIndex: 'form', width: 100, render: (form: TVFormData) => form.date?.toLocaleDateString('en-US') || '-' },
    { title: 'Time', dataIndex: 'form', width: 80, render: (form: TVFormData) => formatWibTimeDisplay(form.time) || '—' },
    { title: 'Duration', dataIndex: 'form', width: 80, render: (form: TVFormData) => form.duration ? `${form.duration}s` : '-' },
    { title: 'Journalist', dataIndex: 'form', width: 100, ellipsis: true, render: (form: TVFormData) => form.journalist || '-' },
    {
      title: 'Status',
      dataIndex: 'index',
      width: 140,
        ...
        <Space wrap size={4}>
          ...
          {multiDuplicateInfo.duplicatesByForm.has(index) && (
            <>
              ...
              <Button type="text" size="small" icon={<EyeOutlined />} onClick={() => { setSelectedDuplicateFormIndex(index); setMultiDuplicateViewOpen(true) }} />
            </>
          )}
        </Space>
      ),
  ...
  return (
    <>
      <PageHeader
        title="Upload TV Article"
        description="Add television articles with video clipping files."
        breadcrumb={[{ title: 'Home', href: '/dashboard' }, { title: 'TV', href: '/tv/list' }, { title: 'Upload' }]}
        extra={
            ...
            <Card key={idx} size="small" style={{ marginBottom: 12 }}>
              <Space orientation="vertical" style={{ width: '100%' }}>
                <Space style={{ justifyContent: 'space-between', width: '100%' }}>
                  ...
                  <span style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>Created: {dup.created_at ? new Date(dup.created_at).toLocaleString('en-US') : '-'}</span>
                </Space>
                ...
                {dup.content_preview && <div style={{ background: '#fafafa', padding: 8, borderRadius: 4, fontSize: 12, maxHeight: 100, overflow: 'auto' }}>{dup.content_preview}</div>}
                <Link href={`/tv/list?search=${encodeURIComponent(dup.title)}`} target="_blank"><Button size="small" icon={<EyeOutlined />}>View in List</Button></Link>
              </Space>
            </Card>
          ))}
        </div>
      ...
      <Modal
        ...
        onCancel={() => { setMultiDuplicateViewOpen(false); setSelectedDuplicateFormIndex(null) }}
        title={`Duplicate Details — Article #${(selectedDuplicateFormIndex ?? 0) + 1}`}
        width={640}
        footer={<Button onClick={() => { setMultiDuplicateViewOpen(false); setSelectedDuplicateFormIndex(null) }}>Close</Button>}
      >
              ...
              {multiDuplicateInfo.duplicatesByForm.get(selectedDuplicateFormIndex)?.map((dup, idx) => (
                <Card key={idx} size="small" style={{ marginBottom: 12 }}>
                  <Space orientation="vertical" style={{ width: '100%' }}>
                    <Space style={{ justifyContent: 'space-between', width: '100%' }}>
                      <Badge count={`#${dup.article_id}`} style={{ backgroundColor: '#faad14' }} />
                      <span style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)' }}>{dup.created_at ? new Date(dup.created_at).toLocaleString('en-US') : '-'}</span>
                    </Space>
                    <strong style={{ color: '#1e3a5f' }}>{dup.title}</strong>
                    <Space><Badge status="default" text={dup.media_name} /><Badge status="default" text={dup.datee} /></Space>
