...
export default function RadioListPage() {
  ...
  useEffect(() => {
    fetch('/api/media/type/by-id/13', { credentials: 'include' })
      .then((r) => (r.ok ? r.json() : []))
      .then((data) =>
        setMediaOptions(
  ...
  const fetchArticles = useCallback(
    async (page = 1, limit = pagination.limit) => {
      ...
      try {
        ...
        const res = await fetch(`/api/articles/radio?${params.toString()}`, {
          credentials: 'include',
          cache: 'no-store',
        ...
        message.error('Failed to load radio articles')
      } finally {
        setLoading(false)
      }
    },
  ...
  const openDuplicatePreview = async (duplicate: DuplicateArticleMatch) => {
    const article = await fetchArticleById('/api/articles/radio', duplicate.article_id)
    if (article) {
      setDupOpen(false)
      openPreview(article)
    } else {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setEditLoading(true)
      const res = await fetch(`/api/articles/radio/${editArticle.article_id}`, {
        method: 'PUT',
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/articles/radio/${deleteTarget.article_id}`, {
        method: 'DELETE',
        credentials: 'include',
      })
      if (res.ok) {
  ...
  return (
    <>
      <PageHeader
        title="Radio Article List"
        description="Browse, preview, and manage radio clipping articles."
        breadcrumb={[{ title: 'Home', href: '/dashboard' }, { title: 'Radio Articles' }]}
        extra={
          <Link href="/radio/upload">
      ...
      <Card variant="borderless" className="digivla-page-card digivla-list-table-card digivla-article-list-card">
        ...
        {shouldShowTableSkeleton(loading, articles.length) ? (
          ...
          <Table<ArticleRow>
            ...
            locale={{ emptyText: 'No radio articles found' }}
          />
        )}
      </Card>
